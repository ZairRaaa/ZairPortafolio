...
(() => {
  ...
  const sync = () => {
    frame.contentWindow?.postMessage({ type: 'huellitas:state', steps: [...steps], theme: document.documentElement.dataset.theme || 'light', palette: document.documentElement.dataset.palette || 'green' }, origin);
  };
  const render = message => {
    toggles.forEach((button, index) => button.setAttribute('aria-pressed', String(steps[index])));
  ...
  toggles.forEach((button, index) => button.addEventListener('click', () => {
    steps[index] = !steps[index];
    document.querySelector('#step-explanation').textContent = descriptions[index];
    render(`Paso ${index + 1} ${steps[index] ? 'activado' : 'desactivado'}.`);
  }));
  document.querySelector('#enable-all').addEventListener('click', () => { steps.fill(true); render('Los cinco pasos están activos.'); });
  document.querySelector('#disable-all').addEventListener('click', () => { steps.fill(false); render('Todo está oculto. Activa un paso para empezar.'); });
  document.querySelectorAll('[data-width]').forEach(button => button.addEventListener('click', () => {
    ...
    document.querySelector('.preview-canvas').classList.toggle('is-mobile', button.dataset.width !== 'full');
  }));
  ...
  window.addEventListener('message', event => {
    ...
    if (data.type === 'huellitas:height' && Number.isFinite(data.height)) frame.style.height = `${Math.max(350, Math.min(15000, data.height))}px`;
    if (data.type === 'huellitas:enable' && Number.isInteger(data.step) && data.step >= 1 && data.step <= 5) {
      steps[data.step - 1] = true;
      render(`Paso ${data.step} activado desde Huellitas.`);
    }
  });
  new MutationObserver(sync).observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme', 'data-palette'] });
  sync();
})();
