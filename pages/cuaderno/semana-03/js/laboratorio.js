...
(() => {
  ...
  const updateEmpty = () => {
    document.querySelector("#empty-preview").hidden = parts.some(
      (part) => !part.hidden,
    );
    document.querySelectorAll("[data-extra]").forEach((extra) => {
      extra.hidden = Boolean(
        parts.find((part) => part.dataset.part === extra.dataset.extra)?.hidden,
      );
    });
  };
  ...
  document.querySelectorAll("[data-favorite]").forEach((button) =>
    button.addEventListener("click", () => {
      ...
      if (favorites.has(dog)) favorites.delete(dog);
      else favorites.add(dog);
      const selected = favorites.has(dog);
      button.setAttribute("aria-pressed", String(selected));
      button.textContent = selected ? "♥" : "♡";
      document.querySelector("#favorite-count").textContent = String(
        favorites.size,
      );
    }),
  );
  ...
  const reportHeight = () => {
    ...
    if (height !== previousHeight) {
      ...
      send({ type: "huellitas:height", height });
    }
  };
  window.addEventListener("message", (event) => {
    ...
    if (
      ...
      parts.forEach((part) => {
        part.hidden = !data.steps[Number(part.dataset.part) - 1];
      });
      updateEmpty();
    }
    if (["light", "dark"].includes(data.theme))
      document.documentElement.dataset.bsTheme = data.theme;
    if (["green", "red"].includes(data.palette))
  ...
  const reveal = (step) => {
    ...
    if (part) {
      part.hidden = false;
      updateEmpty();
      send({ type: "huellitas:enable", step });
    }
  };
    ...
    link.addEventListener("click", () => {
      ...
      if (window.bootstrap)
        bootstrap.Collapse.getInstance(
          document.querySelector("#dog-menu"),
        )?.hide();
    }),
  );
  ...
  const fields = [
    name,
    document.querySelector("#person-email"),
    choice,
    consent,
  ];
  let submitted = false;
  const validateField = (field) => {
    if (field === name)
      name.setCustomValidity(
        name.value.trim().length < 2 ? "Escribe al menos dos caracteres." : "",
      );
    const invalid = !field.validity.valid;
    field.classList.toggle("is-invalid", invalid);
    field.setAttribute("aria-invalid", String(invalid));
    if (field === consent)
      document.querySelector("#consent-error").hidden = !invalid;
    return !invalid;
  };
  ...
  if ("ResizeObserver" in window)
    new ResizeObserver(() => requestAnimationFrame(reportHeight)).observe(
      document.body,
    );
  window.addEventListener("load", () => {
    ...
    if (unavailable) {
      notice.hidden = false;
      notice.textContent =
        "No se cargaron todos los recursos del ejercicio. Revisa tu conexión y recarga para usar el diseño y el menú completos.";
    }
    reportHeight();
  });
